  ...
  ({
    ...
    return (
      ...
      >
        ...
        <span className="relative inline-flex items-center justify-center gap-2 [&_svg]:group-hover:scale-110 [&_svg]:group-hover:rotate-6 [&_svg]:transition-transform [&_svg]:duration-300">
          {children}
        </span>

        {/* Ripple effect background */}
        <span className="absolute inset-0 overflow-hidden rounded-lg opacity-0 group-hover:opacity-20 bg-white transition-opacity duration-300" />

        {/* Glow effect for primary buttons */}
        {variant === "default" && (
          <span className="absolute -inset-1 rounded-lg bg-gradient-to-r from-blue-400 to-purple-400 opacity-0 group-hover:opacity-20 blur-lg transition-opacity duration-500 -z-10" />
        )}

        {/* Success glow */}
        {variant === "success" && (
          <span className="absolute -inset-1 rounded-lg bg-gradient-to-r from-green-400 to-emerald-400 opacity-0 group-hover:opacity-20 blur-lg transition-opacity duration-500 -z-10" />
        )}

        {/* Warning glow */}
        {variant === "warning" && (
          <span className="absolute -inset-1 rounded-lg bg-gradient-to-r from-amber-400 to-orange-400 opacity-0 group-hover:opacity-20 blur-lg transition-opacity duration-500 -z-10" />
        )}
      </Comp>
    )
  }
